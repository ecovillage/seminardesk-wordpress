module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		po2mo: {
			files: {
				src: 'languages/' + pkg.name + '-de_DE.po',
				dest: 'release/' + pkg.version + '/languages/' + pkg.name + '-de_DE.mo'
			}
		},
	...
	grunt.registerTask('php', ['addtextdomain', 'makepot', 'po2mo']);
	grunt.registerTask('default', ['styles', 'scripts', 'php']);

	grunt.registerTask('dist', ['clean:release', 'default', 'replace:readme_txt', 'copy', 'compress']);
	grunt.registerTask('version', ['default', 'replace:version_php', 'replace:version_readme']);
	grunt.registerTask('release', ['dist', 'wp_deploy']);
